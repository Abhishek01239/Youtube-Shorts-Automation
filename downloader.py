...
def download_video(video_id):
    ...
    # Attempt 1: Try with cookies if cookies.txt exists
    ...
    if os.path.exists(cookies_path) and os.path.getsize(cookies_path) > 10:
        ...
        print(f"[*] Downloading {video_id} (Attempt 1: with cookies)...")
        try:
            ...
            print(f"[!] Cookie attempt failed: {e}. Retrying without cookies...")

    # Attempt 2: Download without cookies (fallback)
    print(f"[*] Downloading {video_id} (Attempt 2: fallback without cookies)...")
    try:
